...
import optuna
import csv
from optuna.pruners import NopPruner   # <- add

# Assuming TabularEmbedder, TabularTransformerEncoder, TabularBertPretrainer, TabularBERTModel, and preprocess_table are defined elsewhere
# Import your necessary classes and functions here if they're not already imported

if __name__ == "__main__":
    # Base directories for datasets, splits, and categorical columns
    datasets_dir = "/scratch/pedrobacelar/tabular-transformer/nosso_modelo/processed_datasets"
    splits_dir = "/scratch/pedrobacelar/tabular-transformer/nosso_modelo/processed_datasets/splits"
    cat_cols_dir = "/scratch/pedrobacelar/tabular-transformer/nosso_modelo/cat_num"
    
    # Output directory for results
    output_dir = "/scratch/pedrobacelar/tabular-transformer/nosso_modelo/resultados"
    os.makedirs(output_dir, exist_ok=True)
    
    # Get list of unique dataset names (removing the _XXnan.csv suffix)
    dataset_files = [f for f in os.listdir(datasets_dir) if f.endswith('.csv')]
    dataset_names = list(set([f.split('_')[0] for f in dataset_files]))
    
    # Nan percentages to process
    nan_percentages = ['00', '20', '40', '60', '80']
    
    # Function to load categorical columns from txt file
    def load_categorical_columns(dataset_name):
        categorical_file = os.path.join(cat_cols_dir, f"{dataset_name}.txt")
        categorical_columns = []
    ...
    def save_metrics(dataset_name, nan_percent, metrics, binary_classification):
        file_path = os.path.join(output_dir, f"{dataset_name}_{nan_percent}nan_metrics.csv")
        
        if binary_classification:
            header = ['accuracy', 'precision_macro', 'recall_macro', 'f1_macro', 
                      'TN', 'FP', 'FN', 'TP', 'f1_class0', 'f1_class1']
        else:
            header = ['accuracy', 'precision_macro', 'recall_macro', 'f1_macro']
    ...
    # Function to save best hyperparameters
    def save_hyperparameters(dataset_name, nan_percent, hyperparams):
        file_path = os.path.join(output_dir, f"{dataset_name}_{nan_percent}nan_hyperparams.json")
        with open(file_path, 'w') as f:
            json.dump(hyperparams, f, indent=4)
    
    # Optuna objective function
    def objective(trial, df, splits, categorical_columns, label_column):
        # Track if this is a fixed trial (final evaluation) or an optimization trial
        set_global_seed()
        is_fixed_trial = isinstance(trial, optuna.trial.FixedTrial)
        
        # Hyperparameters to optimize
        DIM = trial.suggest_categorical('DIM', [64, 128, 256, 512])
        HIDDEN_DIM = trial.suggest_categorical('HIDDEN_DIM', [64, 128, 256, 512])
        HEADS = trial.suggest_int('HEADS', 4, 8, step=4)
        LAYERS = trial.suggest_int('LAYERS', 4, 12, step=2)
        DIM_FEED = trial.suggest_categorical('DIM_FEED', [128, 256, 512])
        DROPOUT = trial.suggest_float('DROPOUT', 0.1, 0.3, step=0.1)
        PROB_MASCARA = trial.suggest_float('PROB_MASCARA', 0.2, 0.5, step=0.1)
        LR_PRE = trial.suggest_float('LR_PRE', 1e-4, 1e-3, log=True)
        WEIGHT_DECAY_PRE = trial.suggest_float('WEIGHT_DECAY_PRE', 1e-4, 1e-3, log=True)
        LR_FINE = trial.suggest_float('LR_FINE', 1e-4, 1e-3, log=True)
        WEIGHT_DECAY_FINE = trial.suggest_float('WEIGHT_DECAY_FINE', 1e-4, 1e-3, log=True)
        
        # Fixed hyperparameters
        EPOCHS_PRE = 50  # Reduced for optuna trials
        EPOCH_FINE = 50  # Reduced for optuna trials
        BATCH = 256
        
        # Device configuration
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        
        # Count number of unique labels to determine LABELS
        LABELS = len(df[label_column].unique())
        ...
        # Preprocessing for pre-training (without labels)
        ...
        # Normalize numerical columns if any
        ...
        # Optimizer and scheduler for pre-training
        ...
        # Pre-training loop
        print(f"\n=== Iniciando Pré-Treinamento (Trial {trial.number}) ===")
        for epoch in range(EPOCHS_PRE):
            # Generate new masks for each epoch
            ...
            # Use tqdm if this is a fixed trial (final evaluation)
            if is_fixed_trial:
                range_func = tqdm(range(0, df_train_size, BATCH), 
                                 desc=f"Epoch {epoch+1}/{EPOCHS_PRE}", 
                                 unit="batch")
            else:
                range_func = range(0, df_train_size, BATCH)
            ...
            with torch.no_grad():
                val_loss_sum = 0.0
                val_steps = 0
                for start in range(0, df_val_size, BATCH):
                    end = start + BATCH
            ...
            # Report progress, but only use trial.report if this is not a fixed trial and every 10 epochs
            print(f"Epoch [{epoch+1}/{EPOCHS_PRE}]  Train: {avg_train_loss:.4f} | Val: {avg_val_loss:.4f}")
            
            # Report to Optuna every 10 epochs and not in the final evaluation run
            if not is_fixed_trial and (epoch + 1) % 10 == 0:
        ...
        # Prepare features for fine-tuning
        ...
        classifier_model = TabularBERTModel(
            embedder=pretrain_model.embedder,
        ...
        scheduler_ft = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_ft, T_max=EPOCH_FINE)
        
        best_val_loss = float('inf')
        best_model_state = None
        
        train_losses_ft_history = []
        val_losses_ft_history = []
        
        # Fine-tuning loop
        print(f"\n=== Iniciando Fine-Tuning (Trial {trial.number}) ===")
        for epoch in range(EPOCH_FINE):
            ...
            # Use tqdm if this is a fixed trial (final evaluation)
            if is_fixed_trial:
                range_func = tqdm(range(0, len(processed_train), BATCH), 
                                 desc=f"FineTune Epoch {epoch+1}/{EPOCH_FINE}", 
                                 unit="batch")
            else:
                range_func = range(0, len(processed_train), BATCH)
            ...
            train_losses_ft_history.append(train_loss_epoch)
            ...
            val_losses_ft_history.append(val_loss_value)
            ...
            if is_fixed_trial:
                print(f"\n[Epoch {epoch+1}/{EPOCH_FINE} Summary]")
                ...
                print(f"[Epoch {epoch+1}/{EPOCH_FINE}] Train: {train_loss_epoch:.4f} | Val: {val_loss_value:.4f} | F1 macro: {f1_val_macro:.4f}")
            
            if val_loss_value < best_val_loss:
                best_val_loss = val_loss_value
                best_model_state = {key: value.cpu() for key, value in classifier_model.state_dict().items()}
                if is_fixed_trial:
                    print(f"  New best model saved! (loss: {best_val_loss:.4f})")
            
            # Report to Optuna every 10 epochs and not in the final evaluation run
            if not is_fixed_trial and (epoch + 1) % 10 == 0:
                trial.report(f1_val_macro, epoch + EPOCHS_PRE)  # Offset epoch number to avoid collision
                if trial.should_prune():
                    raise optuna.exceptions.TrialPruned()
        
        # Load best model
        if best_model_state is not None:
            classifier_model.load_state_dict({k: v.to(device) for k, v in best_model_state.items()})
        
        # Evaluate on test set
        classifier_model.eval()
        with torch.no_grad():
            logits_test, _ = classifier_model(processed_test, labels=y_test)
        ...
        f1_macro = f1_score(y_test_np, preds_test_np, average='macro')
        f1_per_class = f1_score(y_test_np, preds_test_np, average=None)
        ...
        if is_fixed_trial:
            ...
            for idx, f1 in enumerate(f1_per_class):
                print(f"F1(class {idx}): {f1:.4f}")
            print("----------------------------------------")
        
        # Return f1_macro as we want to maximize it (Optuna is set to maximize)
        return f1_macro, acc_test, precision_macro, recall_macro, f1_macro, f1_per_class, y_test_np, preds_test_np
    ...
    for dataset_name in dataset_names:
        ...
        categorical_columns = load_categorical_columns(dataset_name)
        
        # Process each nan percentage
        for nan_percent in nan_percentages:
            print(f"  Processing {nan_percent}% NaN")
            
            # Construct the file paths
            dataset_file = f"{dataset_name}_{nan_percent}nan.csv"
            dataset_path = os.path.join(datasets_dir, dataset_file)
            split_path = os.path.join(splits_dir, f"{dataset_name}_split.json")
            
            # Check if files exist
            if not os.path.exists(dataset_path) or not os.path.exists(split_path):
                print(f"    Missing files for {dataset_name} with {nan_percent}% NaN. Skipping.")
                continue
            ...
            label_column = "class"  # Assuming all datasets use "class" as the label column
            ...
            binary_classification = len(label_encoder.classes_) == 2
            
            # Create an Optuna study to maximize f1_macro
            study = optuna.create_study(direction="maximize", pruner=NopPruner())
            
            # Run optimization
            try:
                # Wrap objective to provide additional parameters
                objective_func = lambda trial: objective(trial, df, splits, categorical_columns, label_column)[0]
                
                print(f"\n=== Starting Optuna optimization for {dataset_name}_{nan_percent}nan ===")
                study.optimize(objective_func, n_trials=20, timeout=14400)  # 10 trials or 2 hours max
                ...
                # Run the best model again to get metrics
                print(f"\n=== Running final evaluation with best parameters ===")
                best_result = objective(optuna.trial.FixedTrial(best_params), 
                                        df, splits, categorical_columns, label_column)
                
                # Unpack results
                _, acc_test, precision_macro, recall_macro, f1_macro, f1_per_class, y_test, preds_test = best_result
                
                # Prepare metrics for saving
                if binary_classification:
                    cm = confusion_matrix(y_test, preds_test)
                    tn, fp, fn, tp = cm.ravel()
                    metrics = [acc_test, precision_macro, recall_macro, f1_macro, 
                              tn, fp, fn, tp, f1_per_class[0], f1_per_class[1]]
                else:
                    metrics = [acc_test, precision_macro, recall_macro, f1_macro]
                
                # Save metrics and hyperparameters
                save_metrics(dataset_name, nan_percent, metrics, binary_classification)
                save_hyperparameters(dataset_name, nan_percent, best_params)
                
                print(f"    Completed {dataset_name} with {nan_percent}% NaN.")
                print(f"    Best F1 Macro: {f1_macro:.4f}")
                
            except Exception as e:
                print(f"    Error processing {dataset_name} with {nan_percent}% NaN: {str(e)}")