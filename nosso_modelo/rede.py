...
if __name__ == "__main__":
    ...
    import json
    from pathlib import Path     # se quiser trabalhar com paths

    # -----------------------------------------------------------------
    # Carrega os splits previamente gerados
    # -----------------------------------------------------------------
    
    with open(Path("/scratch/pedrobacelar/tabular-transformer/nosso_modelo/processed_datasets/splits/vehicle_split.json")) as f:   # <= ajuste o caminho
        splits = json.load(f)
    
    df = pd.read_csv("/scratch/pedrobacelar/tabular-transformer/nosso_modelo/processed_datasets/vehicle_80nan.csv")
    categorical_columns = []
    DIM = 256
    HIDDEN_DIM = 256
    ...
    LR_PRE = 0.00010032375968441385
    WEIGHT_DECAY_PRE = 0.00014649565137033754
    
    PROB_MASCARA = 0.2
    
    EPOCH_FINE = 50
    LR_FINE = 0.0001729515798216025
    WEIGHT_DECAY_FINE= 0.0004546086754572825
    ...
    if numerical_columns:                                   # ⬅️  novo guard‑clause
        scaler = StandardScaler()
        df[numerical_columns] = scaler.fit_transform(df[numerical_columns])
    else:
        scaler = None        # (mantém compatibilidade caso você salve o objeto)
    ...
    pretrain_model.parameters(), lr=LR_PRE, weight_decay=WEIGHT_DECAY_PRE)   # lr menor
...
# =================================================
    ...
    best_model_state = None          # manter em memória (rápido)
    ...
    for epoch in range(num_epochs_ft):
        ...
        if val_loss_value < best_val_loss:
            ...
            # torch.save(best_model_state, best_ckpt_path)   # ← se quiser disco
    ...
    if best_model_state is not None:
        ...
        # ou: classifier_model.load_state_dict(torch.load(best_ckpt_path))

    # Agora avalie no conjunto de teste
    classifier_model.eval()
    with torch.no_grad():
